<template>
    <nav class="flex-no-wrap relative flex w-full items-center justify-between bg-[#26841b] py-2 shadow-md lg:flex-wrap lg:justify-start lg:py-4">
      <div class="flex w-full flex-wrap items-center justify-between px-3">
        <!-- Hamburger button for mobile view -->
        <button class="block border-0 bg-transparent px-2 text-neutral-500 hover:no-underline hover:shadow-none focus:no-underline focus:shadow-none focus:outline-none focus:ring-0 dark:text-neutral-200 lg:hidden" type="button" @click="mobileMenuOpen = !mobileMenuOpen" aria-label="Toggle navigation">
          <!-- Hamburger icon -->
          <span class="[&>svg]:w-7">
        ...
        <div class="!visible hidden flex-grow basis-[100%] items-center lg:!flex lg:basis-auto" id="navbarSupportedContent1" data-te-collapse-item>
          <!-- Logo -->
          <router-link to="/">
            <img src="\src\mmsu-logo.png" style="height: 70px" alt="TE Logo" loading="lazy" class="ml-20"/>
          </router-link>
        ...
        <!-- Button for "Book Now" using router-link with right margin -->
        <router-link to="/facilities" class="bg-[#fbbd08] hover:bg-yellow-400 text-black font-bold py-2 px-4 rounded-full  mr-10" data-te-nav-link-ref>Book Now</router-link>
      </div>
    </nav>
</template>
